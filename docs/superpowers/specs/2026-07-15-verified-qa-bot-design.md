...
- **Deferred follow-ups (tickets):** (1) enable caching — move rules+corpus into a `cache_control`-tagged `system[]` block with the question in the user message (`makeAnswerer` → `(system, question) => Promise<string>`). *Trigger revised — see the 2026-07-16 update below: the SDK blocker is gone, but caching is now gated on game-launch volume, not the SDK upgrade.* (2) add `tests/bot/router.test.ts` pinning the `/ask`-before-mod-gate ordering; (3) branch the unset-`verifiedRoleId` copy so it doesn't tell everyone to "verify first" before the role is configured; (4) optional `/config` setter for `verifiedRoleId`/`askLogChannelId` (currently set by direct DB edit per §12).

### Update 2026-07-16 — caching re-evaluated after the SDK upgrade (re-deferred to game launch)

The `@anthropic-ai/sdk` blocker from the v1 note above is **resolved**: the SDK was upgraded `0.32.1 → 0.111.0` (bot `master` `214e513`), so GA `cache_control` on `system[]` blocks is now available. Despite that, **caching was consciously NOT enabled** — on re-evaluation the economics don't favor it yet, so follow-up (1)'s trigger moves from "next SDK upgrade" (done) to **"at game launch, when `/ask` volume is continuous."**

Why re-deferred:

- **Cache writes cost more than not caching.** A cache read is ~0.1× base input, but a cache *write* is **1.25×** (5-min TTL) or **2×** (1-hour TTL). Caching only pays off when questions arrive within the TTL often enough that reads outnumber the write premium — **break-even ≈ 3 questions/hour, sustained.**
- **Pre-launch volume is far below that.** NOPAS is still "Coming to Roblox," so `/ask` traffic is ~tens/day. Nearly every request would land on a cold cache, pay the write premium, and expire before any read — caching would cost **more** than uncached at this volume.
- **Corrected cost figures** (supersede the v1 note's ≈1¢/$150): at Haiku 4.5's $1/M input + $5/M output, a question is ≈ **0.7¢ uncached** (~5.8k input + ~200 output), so ≈ **$102/mo at 500/day**, not $150. The caching saving caps at ≈ **$61/mo**, and only once traffic is busy.
- **Corpus clears the cache floor, but not by much.** The as-built `BASE_KB` (60 entries) measures ~20k chars ≈ **5.0–5.8k tokens** — over Haiku 4.5's 4,096-token minimum cacheable prefix, but only ~25%. Trimming the KB could silently drop it under the floor (where `cache_control` is ignored with no error), so re-measure before enabling.
- **Prompt is already cache-shaped:** `buildAskPrompt` puts rules + corpus first and the question last, so no prompt restructuring is needed when the switch flips — only the `cache_control` breakpoint + the `makeAnswerer` signature change in follow-up (1).

**Decision:** bundle the caching enablement with the game-launch work, alongside the companion bot's Neon-keepalive follow-up (same volume trigger — both only pay off once traffic is continuous). Re-run the `usage.cache_read_input_tokens` > 0 live-check (the struck §12 step) at that point to confirm hits.
